...
function getScriptUrl() {
  ...
  if (configuredUrl && configuredUrl !== DEFAULT_SCRIPT_PLACEHOLDER) {
...
function applyConfig() {
  ...
  if (config.ENABLE_LOCAL_SCRIPT_URL_SETTING === false) {
...
function setConnection(status, text) {
  ...
  els.connectionDot.className = `dot ${status || ""}`.trim();
...
function toggleSetupWarning() {
  if (isScriptConfigured()) {
    ...
    setConnection("ok", "Backend aktif");
  } else {
    els.setupWarning.classList.remove("hidden");
    setConnection("bad", "Backend belum siap");
  }
}
...
function setLoading(isLoading) {
  ...
  els.submitBtn.innerHTML = isLoading ? "<span>Menyimpan...</span>" : "<span>Simpan ke Google Sheet</span><b>↗</b>";
}
...
async function refreshAll(showLoading = false) {
  ...
  try {
    ...
    if (keysResult.status === "fulfilled" || membersResult.status === "fulfilled") {
      setConnection("ok", "Backend aktif");
    } else {
      setConnection("bad", "Gagal memuat");
    }
  } finally {
